...
pub fn main() !void {
    ...
    while (it.next()) |entry| {
        ...
        // Track this model's triangle range
        const name = try allocator.dupe(u8, entry.file_name);
...

    {
        ...
        // Compute which models overlap each cluster for runtime culling
        {
            try stdout.print("\n  Computing cluster→model mapping...\n", .{});
            ...
            // For each cluster, collect overlapping models
            var cluster_model_sets = try allocator.alloc(std.AutoHashMap(u32, void), final_cluster_count);
            defer {
                for (cluster_model_sets) |*cms| cms.deinit();
                allocator.free(cluster_model_sets);
            }
            for (0..final_cluster_count) |ci| {
                cluster_model_sets[ci] = std.AutoHashMap(u32, void).init(allocator);
            }
            ...
            for (0..final_cluster_count) |ci| {
                ...
                for (start_t..end_t) |sorted_idx| {
                    // Map BIVH-sorted index back to original triangle
                    ...
                    if (model_id != std.math.maxInt(u32)) {
                        try cluster_model_sets[ci].put(model_id, {});
                    }
                }
            }

            // Count unique models across all clusters
            var unique_models = std.AutoHashMap(u32, void).init(allocator);
            defer unique_models.deinit();
            for (cluster_model_sets) |*cms| {
                var it = cms.keyIterator();
                while (it.next()) |key| {
                    try unique_models.put(key.*, {});
                }
            }

            try stdout.print("  Models per cluster: min/max/avg\n", .{});
            var min_models: u32 = std.math.maxInt(u32);
            var max_models: u32 = 0;
            var total_models: u64 = 0;
            for (cluster_model_sets) |*cms| {
                const count: u32 = @intCast(cms.count());
                min_models = @min(min_models, count);
                max_models = @max(max_models, count);
                total_models += count;
            }
            const avg_models: u32 = @intCast(total_models / final_cluster_count);
            try stdout.print("    {d} / {d} / {d}\n", .{ min_models, max_models, avg_models });
            try stdout.print("  Unique models: {d}/{d}\n", .{ unique_models.count(), model_ranges.items.len });

            // Write to binary: cluster_models.bin
            const cm_bin = try std.fmt.allocPrint(allocator, "{s}_cluster_models.bin", .{base_name});
            defer allocator.free(cm_bin);
            var cmf = try std.fs.cwd().createFile(cm_bin, .{});
            defer cmf.close();
            var cmw = std.io.bufferedWriter(cmf.writer());
            const w = cmw.writer();

            try w.writeAll("CMOD"); // magic
            try w.writeInt(u32, final_cluster_count, .little);
            try w.writeInt(u32, @intCast(model_ranges.items.len), .little);

            for (cluster_model_sets, 0..) |*cms, ci| {
                try w.writeInt(u32, @intCast(ci), .little);
                try w.writeInt(u32, @intCast(cms.count()), .little);

                var it = cms.keyIterator();
                while (it.next()) |key| {
                    try w.writeInt(u32, key.*, .little);
                }
            }
            try cmw.flush();
            ...
            // Also write model names for reference
